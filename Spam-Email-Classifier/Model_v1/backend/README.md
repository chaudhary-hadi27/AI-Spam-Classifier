...
## 🔧 Clone Backend Only (Optional)
### I recommend cloning the entire project along with all files for full functionality.
**But if you only want the backend, here are the steps to do that:**
- **⚠️ Note You need dataset for training model which is define in `Model_v1/backend/model.py`.**
- The file `app.py` contains the API for the frontend `(Model_v1/frontend)`.
- If you're cloning only the backend, `app.py` is not required.
- Ensure all paths are correct, since some may reference directories outside the backend folder.
